...
| 오류 코드 | 원인 | 해결 방법 |
|---|---|---|
| `Unsupported provider: provider is not enabled` | Supabase Kakao Provider 미활성화 | 대시보드 → Authentication → Providers → Kakao 토글 ON + REST API Key / Client Secret 입력 |
| 카카오 KOE004 (앱 관리자 설정 오류) | 카카오 개발자 콘솔 Web 플랫폼 도메인 미등록 | 플랫폼 → Web → `https://alicelimti.github.io` · `https://brwtobsmjyfsbhowhsmg.supabase.co` 등록 |
| 카카오 KOE205 (Redirect URI 불일치) | 카카오 앱에 등록된 Redirect URI와 Supabase가 전송하는 URI가 불일치 | 카카오 로그인 → Redirect URI를 `https://brwtobsmjyfsbhowhsmg.supabase.co/auth/v1/callback` 으로 정확히 등록 (trailing slash, 오탈자 주의) |
